<?php 
	...
	#Edit user profile
		...
		if ($_SESSION['user']['role'] == 1) {
			...
			<form action="" id="reg_form" name="registration_form" method="POST">
				...
				<label for="role">role<small>1-admin  2-editor  3-user</small></label>
				<input type="number" id="role" name="role" min="1" max="3" value="' . $row['role'] . '" placeholder="Role" required><br>
				<hr>
				...
				<table>
					...
					<tbody>';
						...
						while($row = @mysqli_fetch_array($result)) {
							print '
							<tr>
								<td><a href="index.php?menu='.$menu.'&amp;action='.$action.'&amp;id=' .$row['id']. '"><p>User</p></a></td>
								<td>';
									if ($_SESSION['user']['role'] == 1) {
										print '<a href="index.php?menu='.$menu.'&amp;action='.$action.'&amp;edit=' .$row['id']. '"><p>Edit</p></a></td>';
									}
								print '
								<td>';
									if ($_SESSION['user']['role'] == 1) {
										print '<a href="index.php?menu='.$menu.'&amp;action='.$action.'&amp;delete=' .$row['id']. '"><p>Delete</p></a>';
									}
								print '	
								</td>
