<?php 
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'add_news') {		
			...
			if ($_SESSION['user']['role'] == 3){$_POST['archive']='Y';}
	...
	#Add news 
		...
		<form action="" id="news_form" name="news_form" method="POST" enctype="multipart/form-data">
			...
			else{print'<input type="radio" name="archive" value="Y" required> YES &nbsp;&nbsp;';}
			print'
			<hr>
	...
	#Edit news
	else if ((isset($_GET['edit']) && $_GET['edit'] != '') && ($_SESSION['user']['role'] == 1 || $_SESSION['user']['role'] == 2)) {
		$query  = "SELECT * FROM news";
		...
		<div>
			<table>
				...
				while($row = @mysqli_fetch_array($result)) {
					print '
					<tr>
						<td><a href="index.php?menu='.$menu.'&amp;action='.$action.'&amp;id=' .$row['id']. '"><p>News</p></td>';
						if($_SESSION['user']['role'] == 1  || $_SESSION['user']['role'] == 2){
						print '<td><a href="index.php?menu='.$menu.'&amp;action='.$action.'&amp;edit=' .$row['id']. '"><p>Edit</p></a></td>';
						}
						else{print'<td>not available!</td>';}					
						if($_SESSION['user']['role'] == 1){
						print'<td><a href="index.php?menu='.$menu.'&amp;action='.$action.'&amp;delete=' .$row['id']. '"><p>Delete</p></td>';
						}
						else{print'<td>not available!</td>';}
						print'
						<td>' . $row['title'] . '</td>
			...
			<a href="index.php?menu=' . $menu . '&amp;action=' . $action . '&amp;add=true" class="AddNews">Add news</a>
		</div>';
	}
